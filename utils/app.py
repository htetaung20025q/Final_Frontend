...
async def auth_register(user: user_schema.UserCreate, db: AsyncSession = Depends(get_db)):
...
async def auth_refresh(body: user_schema.RefreshTokenRequest, db: AsyncSession = Depends(get_db)):
...
async def list_products(
    ...
    return await product_service.get_products(category_id, min_price, max_price, available, db)
